...
namespace EquipMainUi.Struct.Detail.EFEM
{
    public class EFEMUnit : StepBase
    {
        ...
        public EFEMSIGLMDataSet TL_None;
        public EFEMSIGLMDataSet TL_Red;
        public EFEMSIGLMDataSet TL_Yellow;
        public EFEMSIGLMDataSet TL_Green;

        public bool IsError;

        EmEfemTowerLampState EFEMTowerLamp;
        ...
        public EFEMUnit()
        {
            ...
            TL_None = new EFEMSIGLMDataSet(EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF);
            TL_Red = new EFEMSIGLMDataSet(EmEfemLampBuzzerState.ON, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF);
            TL_Yellow = new EFEMSIGLMDataSet(EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.ON, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF);
            TL_Green = new EFEMSIGLMDataSet(EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.ON, EmEfemLampBuzzerState.OFF, EmEfemLampBuzzerState.OFF);
        ...
        private void LightCurtainLogic(Equipment equip)
        {
            ...
            {
                if (LPMLightCurtain.IsMuting == false 
                    && LPMLightCurtain.UseAlarm
                    && LPMLightCurtain.Detect.IsOn == true)
                {
        ...
        public override void StatusLogicWorking(Equipment equip)
        {
            ...
            // 230227 신규 추가
            EFEMTowerLampLogic(equip);
            SafetyLogic(equip);
        }

        private void EFEMTowerLampLogic(Equipment equip)
        {
            if (EFEMTowerLamp != EmEfemTowerLampState.Red && equip.IsHeavyAlarm)
            {
                EFEMTowerLampChange(equip, EFEMTowerLamp = EmEfemTowerLampState.Red);
            }
            if (EFEMTowerLamp != EmEfemTowerLampState.Yellow && equip.EquipRunMode == EmEquipRunMode.Manual && !equip.IsHeavyAlarm)
            {
                EFEMTowerLampChange(equip, EFEMTowerLamp = EmEfemTowerLampState.Yellow);
            }
            if (EFEMTowerLamp != EmEfemTowerLampState.Green && equip.EquipRunMode == EmEquipRunMode.Auto && !equip.IsHeavyAlarm)
            {
                EFEMTowerLampChange(equip, EFEMTowerLamp = EmEfemTowerLampState.Green);
            }
        }

        private void EFEMTowerLampChange(Equipment equip, EmEfemTowerLampState Status)
        {
            if (Status == EmEfemTowerLampState.Red)
            {
                if (equip.Efem.Proxy.StartLampBuzzerChange(equip, TL_Red)==false) return;
            }
            else if (Status == EmEfemTowerLampState.Yellow)
            {
                if (equip.Efem.Proxy.StartLampBuzzerChange(equip, TL_Yellow) == false) return;
            }
            else if (Status == EmEfemTowerLampState.Green)
            {
                if (equip.Efem.Proxy.StartLampBuzzerChange(equip, TL_Green) == false) return;
            }
        }
