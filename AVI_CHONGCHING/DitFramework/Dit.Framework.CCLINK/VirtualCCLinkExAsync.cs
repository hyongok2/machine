...
{
    public class VirtualCCLinkExAsync : IVirtualMem
    {
        ...
        public bool[] TEMP_WORD_X = new bool[PLC_MEM_SIZE];
        public bool[] TEMP_WORD_Y = new bool[PLC_MEM_SIZE];
        public bool[] TEMP_WORD_B = new bool[PLC_MEM_SIZE];
        public bool[] TEMP_WORD_SB = new bool[PLC_MEM_SIZE];
        public short[] TEMP_WORD_R = new short[PLC_MEM_SIZE];
        public short[] TEMP_WORD_D = new short[PLC_MEM_SIZE];
        public short[] TEMP_WORD_W = new short[PLC_MEM_SIZE];
        public short[] TEMP_WORD_ZR = new short[PLC_MEM_SIZE];
        public short[] TEMP_WORD_Ww = new short[PLC_MEM_SIZE];
        public short[] TEMP_WORD_Wr = new short[PLC_MEM_SIZE];
        ...
        public List<KeyValuePair<PlcAddr, SyncType>> RearTimeMem = new List<KeyValuePair<PlcAddr, SyncType>>();

        public VirtualCCLinkExAsync(short channel, short stationNo, short md, string name)
        {
            StationNo = stationNo;
            Channel = channel;
            Mode = md;
        ...
        public void Run()
        {
            while (_running)
            {
                if (Monitor.TryEnter(lockObject))
                {
                    try
                    {
                        foreach (KeyValuePair<PlcAddr, SyncType> addr in RearTimeMem)
                        {
                            if (_isFirst)
                            {
                                read(addr.Key, addr.Key.Length);
                            }
                            else if (addr.Value == SyncType.READ)
                            {
                                read(addr.Key, addr.Key.Length);
                            }
                            else if (addr.Value == SyncType.WRITE)
                            {
                                write(addr.Key, addr.Key.Length);
                            }
                        }
                        _isFirst = false;                        
                    }
        ...
        private int read(PlcAddr addr, int wordSize)
        {
            int result = 0;
            if (addr.Type == PlcMemType.B)
            {
                bool[] currWords = GetBists(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_B, addr.Addr, wordSize * 16);
                return result;
            }
            if (addr.Type == PlcMemType.SB)
            {
                bool[] currWords = GetBists(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_SB, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.X)
            {
                bool[] currWords = GetBists(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_X, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.Y)
            {
                bool[] currWords = GetBists(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_Y, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.R)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_R, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.D)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_D, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.ZR)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_ZR, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.W)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_W, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Wr)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_Wr, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Ww)
            {
                short[] currWords = GetShorts(addr, wordSize, out result);
                Array.Copy(currWords, 0, TEMP_WORD_Ww, addr.Addr, wordSize);
                return result;
            }
            else
            {
        ...
        private int write(PlcAddr addr, int wordSize)
        {
            int result = 0;
            ...
            if (addr.Type == PlcMemType.B)
            {
                ...
                short[] shorts = TEMP_WORD_B.GetShorts(addr.Addr, wordSize);
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref shorts[0]);
                return result;
            }
            else if (addr.Type == PlcMemType.X)
            {
                short[] shorts = TEMP_WORD_X.GetShorts(addr.Addr, wordSize);
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref shorts[0]);
                return result;
            }
            else if (addr.Type == PlcMemType.Y)
            {
                short[] shorts = TEMP_WORD_Y.GetShorts(addr.Addr, wordSize);
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref shorts[0]);
                return result;
            }
            else if (addr.Type == PlcMemType.R)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_R[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.D)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_D[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.ZR)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_ZR[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.W)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_W[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.Wr)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_Wr[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.Ww)
            {
                result = mdSendEx(_channelPath, NetworkNo, StationNo, (int)addr.Type, addr.Addr, ref byteSize, ref  TEMP_WORD_Ww[addr.Addr]);
                return result;
            }
            else
            {
        ...
        public override int ReadFromMEM(PlcAddr addr, int wordSize)
        {
            int result = 0;
            if (addr.Type == PlcMemType.B)
            {
                Array.Copy(TEMP_WORD_B, addr.Addr, WORD_B, addr.Addr, wordSize * 16);
                return result;
            }
            if (addr.Type == PlcMemType.SB)
            {
                Array.Copy(TEMP_WORD_SB, addr.Addr, WORD_SB, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.X)
            {
                Array.Copy(TEMP_WORD_X, addr.Addr, WORD_X, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.Y)
            {
                Array.Copy(TEMP_WORD_Y, addr.Addr, WORD_Y, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.R)
            {
                Array.Copy(TEMP_WORD_R, addr.Addr, WORD_R, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.D)
            {
                Array.Copy(TEMP_WORD_D, addr.Addr, WORD_D, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.ZR)
            {
                Array.Copy(TEMP_WORD_ZR, addr.Addr, WORD_ZR, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.W)
            {
                Array.Copy(TEMP_WORD_W, addr.Addr, WORD_W, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Wr)
            {
                Array.Copy(TEMP_WORD_Wr, addr.Addr, WORD_Wr, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Ww)
            {
                Array.Copy(TEMP_WORD_Ww, addr.Addr, WORD_Ww, addr.Addr, wordSize);
                return result;
            }
            else
            {
        ...
        public override int WriteToMEM(PlcAddr addr, int wordSize)
        {
            int result = 0;
            if (addr.Type == PlcMemType.B)
            {
                Array.Copy(WORD_B, addr.Addr, TEMP_WORD_B, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.X)
            {
                Array.Copy(WORD_X, addr.Addr, TEMP_WORD_X, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.Y)
            {
                Array.Copy(WORD_Y, addr.Addr, TEMP_WORD_Y, addr.Addr, wordSize * 16);
                return result;
            }
            else if (addr.Type == PlcMemType.R)
            {
                Array.Copy(WORD_R, addr.Addr, TEMP_WORD_R, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.D)
            {
                Array.Copy(WORD_D, addr.Addr, TEMP_WORD_D, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.ZR)
            {
                Array.Copy(WORD_ZR, addr.Addr, TEMP_WORD_ZR, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.W)
            {
                Array.Copy(WORD_W, addr.Addr, TEMP_WORD_W, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Wr)
            {
                Array.Copy(WORD_Wr, addr.Addr, TEMP_WORD_Wr, addr.Addr, wordSize);
                return result;
            }
            else if (addr.Type == PlcMemType.Ww)
            {
                Array.Copy(WORD_Ww, addr.Addr, TEMP_WORD_Ww, addr.Addr, wordSize);
                return result;
            }
            else
            {
