    ...
    public partial class FrmOperating : Form
    {
        ...
        private void btnWaferReturn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if(item.ToPort.ToString() == "LOADPORT1" || item.ToPort.ToString() == "LOADPORT2")
                    {
                        Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 복귀 전] Port No : {0}, slotProcessCount : {1}", item.ToPort, GG.Equip.CtrlSetting.slotProcessCount);

                        GG.Equip.IsTransferToLoad = true;
                        //GG.Equip.CtrlSetting.slotProcessCount--;
                        //GG.Equip.CtrlSetting.Save();
                        //
                        //Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 복귀 후] Port No : {0}, slotProcessCount : {1}", item.ToPort, GG.Equip.CtrlSetting.slotProcessCount);
                    }
