...
namespace EquipMainUi
{
    public partial class FrmTester : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            btnInspRepeatModeOn.OnOff = GG.Equip.CtrlSetting.InspRepeatMode;
            btnInspRepeatModeOff.OnOff = !GG.Equip.CtrlSetting.InspRepeatMode;

            lblInspRepeatTotal.Text = (GG.Equip.CtrlSetting.InspRepeatCount + 1).ToString();
        }
        ...
        private void btnInspRepeatModeOn_Click(object sender, EventArgs e)
        {
            ButtonDelay2 btn = sender as ButtonDelay2;
            if (btn == btnInspRepeatModeOn)
            {
                if(lblInspRepeatCount.Text == "")
                {
                    MessageBox.Show("반복성 횟수를 입력해주세요.");
                    return;
                }
                else
                {
                    lblInspRepeatCount.Enabled = false;
                    GG.Equip.CtrlSetting.InspRepeatCount = Convert.ToInt16(lblInspRepeatCount.Text);
                    GG.Equip.CtrlSetting.InspRepeatMode = true;
                }
            }
            else if (btn == btnInspRepeatModeOff)
            {
                lblInspRepeatCount.Enabled = true;
                GG.Equip.CtrlSetting.InspRepeatMode = false;
                GG.Equip.CtrlSetting.InspRepeatCount = 0;
            }
            Logger.Log.AppendLine(LogLevel.Info, "InspRepeatMode " + (GG.Equip.CtrlSetting.InspRepeatMode == true ? "Use" : "Unuse"));
