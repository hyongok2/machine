    ...
    public partial class ucrlRobotEasyController : UserControl
    {
        ...
        private EmEfemDBPort GetPort(bool isFrom)
        {
            if (isFrom)
            {
                ...
                {
                    Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 투입 전] slotProcessCount : {0}", GG.Equip.CtrlSetting.slotProcessCount);
                    GG.Equip.IsTransferFromLoad = true;
            ...
            else
            {
                ...
                {
                    Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 복귀 전] slotProcessCount : {0}", GG.Equip.CtrlSetting.slotProcessCount);

                    GG.Equip.IsTransferToLoad = true;
                    //GG.Equip.CtrlSetting.slotProcessCount--;
                    //GG.Equip.CtrlSetting.Save();
                    //
                    //Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 복귀 후] slotProcessCount : {0}", GG.Equip.CtrlSetting.slotProcessCount);
