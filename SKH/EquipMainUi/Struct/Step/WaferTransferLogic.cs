            ...
            switch (_seqStepNum)
            {
                ...
                case EmWaferTransferSeqStep.S040_PICK_START:
                    if (PickCmd != null)
                    {
                        ...
                        if (GG.Equip.IsTransferFromLoad)
                        {
                            GG.Equip.IsTransferFromLoad = false;

                            GG.Equip.CtrlSetting.slotProcessCount++;
                            GG.Equip.CtrlSetting.Save();
                            Logger.TransferDataLog.AppendLine(LogLevel.NoLog, "[Wafer 메뉴얼 투입 후] slotProcessCount : {0}", GG.Equip.CtrlSetting.slotProcessCount);
                        }
