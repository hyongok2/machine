...
namespace EquipMainUi.Struct
{
    ...
    public enum EM_AL_LST
    {
        ...
        AL_0359_INSP_PC_STATS_TOOL_START_ACK_TIMEOVER,
        ...
        AL_0379_INSP_PC_STATS_TOOL_START_HS_TIMEOVER,
        ...
        AL_0398_INSP_PC_EVT_STATS_TOOL_START_ACK_IS_NOT_OFF_ERROR,
        AL_0399_,
        AL_0400_,
        AL_0401_,
        AL_0402_,
        ...
        AL_0679_SLOT_PROCESS_COUNT_ABNORMAL,
        //OverTime 680~769
        ...
        AL_0943_,
        AL_0944_,
        AL_0945_DEEP_LEARNING_COMPLETE_OVERTIME,
            ...
            if (HappenAlarms.ContainsKey(id))
            {
                if (HappenAlarms[id].Happen == false)
                {
                    ...
                    if (HappenAlarms[id].State == EM_AL_STATE.Heavy)
                    {
                        ...
                        if (GG.Equip.Efem.LoadPort1.SeqStepNum == Detail.EFEM.Step.EmEFEMLPMSeqStep.S535_REVIEW_JUDGE_ON || GG.Equip.Efem.LoadPort2.SeqStepNum == Detail.EFEM.Step.EmEFEMLPMSeqStep.S535_REVIEW_JUDGE_ON)
                        {
                            GG.DeepReviewWaitAlarm = true;
                        }

                        if (GG.Equip.PioLPM1.IsRunning || GG.Equip.PioLPM2.IsRunning)
                        {
                            if (GG.Equip.PioLPM1.IsRunning && GG.Equip.IsOhtUnloadRunning)
                            {
                                GG.Equip.LoadPort = 1;

                                GG.Equip.PioWorkingAlarm = true;
                                GG.Equip.IsPauseUnldPioRunning = true;
                                GG.Equip.UnldPioWorkingAlarm = true;

                                Logger.Log.AppendLine(LogLevel.Error, "============== Load Port1 배출(UnLoad) Pio중 알람 발생함 ==============");
                            }
                            else
                            {
                                if (GG.Equip.IsOhtUnloadRunning)
                                {
                                    GG.Equip.LoadPort = 2;
                                    ...
                                    Logger.Log.AppendLine(LogLevel.Error, "============== Load Port2 배출(UnLoad) Pio중 알람 발생함 ==============");
                                }
                            }
                          
                        }
                        else if ((GG.Equip.PioLPM1.IsRunning || GG.Equip.PioLPM2.IsRunning) && equip.IsOhtLoadRunning)
                        {
                            GG.Equip.PioWorkingAlarm = true;
                            GG.Equip.IsPauseLdPioRunning = true;
                            Logger.Log.AppendLine(LogLevel.Error, "============== 투입(Load) Pio중 알람 발생함 ==============");
                        }
                        else
                        {
                            equip.IsPause = true;
