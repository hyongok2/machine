...
{
    public class VirtualCCLink : IVirtualMem
    {
        ...
        public VirtualCCLink(short stationNo, short channel, string name)
        {
            StationNo = stationNo;
            Channel = channel;
            Name = name;
        ...
        {
            short mMD = -1;
            short result = mdOpen(Channel, mMD, ref _channelPath);
            return result;
        }
        ...
        public override int WriteToPLC(PlcAddr addr, int wordSize)
        {
            ...
            if (addr.Type == PlcMemType.B)
            {
                ...
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref shorts[0]);
            ...
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_R[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.D)
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_D[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.ZR)
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_ZR[addr.Addr]);

                return result;
            }
            else if (addr.Type == PlcMemType.W)
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_W[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.Wr)
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_Wr[addr.Addr]);
                return result;
            }
            else if (addr.Type == PlcMemType.Ww)
            {
                result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref WORD_Ww[addr.Addr]);
                return result;
            }
            else
            {
        ...
        {
            short[] readBuff = new short[10];
            short address = (short)addr.Addr;
            short wordSize = 2;
            int result = mdReceive(_channelPath, (short)StationNo, (short)addr.Type, (short)address, ref wordSize, ref readBuff[0]);
            if (addr.Type == PlcMemType.X || addr.Type == PlcMemType.Y || addr.Type == PlcMemType.B)
            {
                return readBuff[0] == 1 ? true : false;
            }
            else
            {
        ...
        public override int SetAscii(PlcAddr addr, string text)
        {
            ...
            short byteSize = (short)(writeBuff.Length * 2);
        ...
        public override void SetShorts(PlcAddr addr, short[] values, out int result)
        {
            ...
            result = mdSend(_channelPath, StationNo, (short)addr.Type, (short)addr.Addr, ref byteSize, ref writeBuff[0]);
        }
